...
function agrega_producto_reclamo(id){
	...
	if (isNaN(cantidad)){
		...
		return false;
			...
			$.ajax({
		    url: './ajax/add_reclamos.php?action=add_tmpprod_vendedor&cveart='+cve_art+'&producto='+producto+'&cantidad='+reclamo+'&cveclie='+cve_clie+'&usuario='+usuario+'&nfactura='+nfactura+'&treclamo='+cantidadtmp+'&tfactura='+cantidad+'&productotmp='+productotmp+'&lotetmp='+lote,
			 beforeSend: function(objeto){
				$("#resultados").html("Mensaje: Cargando...");
			  },
		    success: function(datos){
