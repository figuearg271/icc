...
if($action == 'add_tmpprod_vendedor') // agrega  producto del tmp reclamos det desde panel vendedores
{
	...
	if (($ttmp+$cantidad)>$tfactura && $prod=$prodtmp) {
		...
		try
		{
			...
			$query = "select cve_art,producto,sum(cantidad) as cantidad from tmp_det_reclamos where ltrim(cve_clie)='".ltrim($cliente)."' and usuario='".$usuario."' and estado=0 group by cve_art,producto; ";
		...
		$sql = "INSERT INTO tmp_det_reclamos (cve_art,producto,cantidad,cve_clie,usuario,estado) VALUES ('".trim($cve_art)."','".ltrim($prod)."','".$cantidad."','".ltrim($cliente)."','".ltrim($usuario)."','0')";
...
if($action == 'delet_prod_tmp_vendedores') // elimina producto del tmp reclamos det desde panel vendedores
{
	...
	try
	{
		...
		$query = "select cve_art,producto,cantidad from tmp_det_reclamos where ltrim(cve_clie)='".ltrim($cliente)."' and usuario='".$usuario."'  and estado=0 ; ";
        	...
        	while($row = sqlsrv_fetch_array($stmt))
			{
				...
				<tr>
					...
					<td class='text-center'>
						...
						<a href="#" onclick="delet_prodtmpvendedores('<?php echo $codigo; ?>')"><i class="glyphicon glyphicon-trash"></i></a>
					</td>
				</tr> <?php
			} ?>
			<tr>
...
if($action == 'agrega_reclamo_vendedor') // agrega  producto del tmp reclamos det desde panel vendedores
{
	...
	try
	{
		...
		$sql = "INSERT INTO det_reclamos (ir_reclamo,cve_art,producto,cantidad) select ".$id_reclamo." as ir_reclamo,cve_art,producto,cantidad from tmp_det_reclamos where ltrim(cve_clie)='".ltrim($idcliente)."' and usuario='".ltrim($idusuario)."' and estado=0; ";
